...
const QC_TAX_RATE  = GST_RATE + QST_RATE  // 0.14975
...
function getTaxBreakdown(invoice) {
  ...
  // GST-only provinces (AB, BC, MB, SK, etc.)
...
export default function TaxSummary({ session }) {
  ...
  const totals = filtered.reduce(
    (acc, inv) => {
      ...
      acc.totalTaxCRA    = round2(acc.gstToCRA       + acc.hstToCRA)
      acc.quebecRevenue  = inv.province?.toUpperCase() === 'QC'
        ? round2(acc.quebecRevenue + taxable)
        : acc.quebecRevenue
      return acc
    },
    { grossRevenue: 0, gstToCRA: 0, hstToCRA: 0, qstToRevQC: 0, totalTaxCRA: 0, quebecRevenue: 0 }
  )
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 28 }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 14 }}>
              Revenue breakdown for CRA and Revenue Québec
            </p>
          </div>
          <button onClick={handlePrint} className="btn btn-primary" style={{ gap: 6, display: 'flex', alignItems: 'center' }}>
          ...
          {filtered.length === 0 ? (
            ...
            <>
              ...
              <section style={{ marginBottom: 32 }}>
                ...
                <InvoiceTable
                  ...
                  footerCells={[
                    ...
                    formatCAD(totals.gstToCRA),
                    formatCAD(totals.hstToCRA),
                  ]}
                />
              </section>

              {/* ── Revenue Québec Section ── */}
              <section style={{ marginBottom: 32 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                ...
                {totals.quebecRevenue === 0 ? (
                  ...
                  <>
                    ...
                    <InvoiceTable
                      ...
                      footerCells={[
                        '', '', 'TOTAL',
                        formatCAD(totals.quebecRevenue),
                        formatCAD(totals.gstToCRA),
                        formatCAD(totals.qstToRevQC),
                      ]}
                    />
                  </>
                )}
              ...
              {filtered.some(inv => !inv.gst_enabled || !inv.gst_amount) && (
                <section style={{ marginBottom: 32 }}>
                  <h3 style={{ fontSize: 15, fontWeight: 600, color: '#64748b', marginBottom: 12 }}>
                    Invoices without tax collected
                  </h3>
                  <InvoiceTable
